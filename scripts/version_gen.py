...
try:
    ...
    # PIOENV is the active PlatformIO env name (e.g. `lolin_d32_pro`,
    # `metro_s3`, `test`). Used as the firmware's variant string in POLL_ACK
    # — the server matches release assets named `astros-esp-<v>-<variant>-app.bin`.
    _PIO_ENV = env.subst("$PIOENV")  # type: ignore[name-defined]  # noqa: F821
except NameError:
...
def resolve_variant() -> str:
    """Return the PlatformIO env name for this build.

    When run as a PlatformIO pre-build hook, this is the canonical env name
    (`lolin_d32_pro`, `metro_s3`, `test`). When run as a standalone CLI
    (no PIO context), there is no active env — return `native` as a
    safe placeholder; CLI runs are diagnostic and don't produce a binary
    that needs a real variant.
    """
    return _PIO_ENV if _PIO_ENV else "native"
...
def write_header(version: str, sha: str, variant: str) -> bool:
    ...
    content = (
        ...
        f'    constexpr const char *Variant = "{variant}";\n'
        "}\n"
    )
