...
- [ ] **Make `OtaReceiver::active_` atomic + expose `isActive()`.** In `lib/OtaReceiver/include/OtaReceiver.hpp`: change `bool active_ = false;` to `std::atomic<bool> active_{false};` (add `#include <atomic>`). Add `bool isActive() const noexcept { return active_; }` to the public section, with a comment noting it's safe to call from any task. Existing assignments / conditionals in `OtaReceiver.cpp` keep working unchanged via `std::atomic<bool>`'s conversion operators — no functional change to the receiver's logic, just a type swap for thread safety.

- [ ] **Gate the master polling branch in `pollingTimerCallback`.** In `src/main.cpp:409`: after the heartbeat log line and the `master`/`discovery` snapshot, early-return from the master-mode polling block if `AstrOs_OtaReceiver.isActive()`. Keep the heartbeat log unconditional (cheap, useful for liveness debugging during a flash). Keep the display-timeout decrement unconditional (also cheap, and OLED should still time out during a long OTA). Add a brief comment explaining why we skip: "wire-rate FW_CHUNK processing on otaReceiverTask is sensitive to CPU + UART-TX contention from the 2-s poll cycle."

- [ ] **Build both boards + native tests.** `pio run -e metro_s3` (production), `pio run -e lolin_d32_pro` (other supported board), `pio test -e test` (PURE / native — should be unaffected since OtaReceiver is HARDWARE-MIXED, not in the native build).
